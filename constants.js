...
export const UTILS = {
    ...
    formatNumber: (number, decimals = 2) => new Intl.NumberFormat('en-US',{minimumFractionDigits: 0,maximumFractionDigits: decimals}).format(number),
    formatCurrency: (amount, currency = 'ETH') => `${UTILS.formatNumber(amount, 4)} ${currency}`,
    formatPercentage: value => `${UTILS.formatNumber(value * 100, 1)}%`,
    ...
    truncateText: (text, maxLength) => (!text || text.length <= maxLength) ? text : text.slice(0, maxLength) + '...',
    calculateQuorumPercentage: (forVotes, againstVotes, totalMembers) => totalMembers > 0 ? ((forVotes + againstVotes) / totalMembers) * 100 : 0,
    hasProposalPassed: (forVotes, againstVotes, quorumRequired = 0.1) => {
        const totalVotes = forVotes + againstVotes;
        return totalVotes && forVotes > againstVotes && (forVotes / totalVotes) >= quorumRequired;
    },
    ...
    sortByKey: (array, key, ascending = true) => [...array].sort((a, b) => {
        const valueA = a[key], valueB = b[key], comparison = valueA < valueB ? -1 : valueA > valueB ? 1 : 0;
    ...
    generateTransactionLink: txHash => `${CONTRACT_CONFIG.network.explorer}/tx/${txHash}`,
    generateAddressLink: address => `${CONTRACT_CONFIG.network.explorer}/address/${address}`,
    ...
    sanitizeInput: input => typeof input !== 'string' ? input : input.trim().replace(/[<>]/g, ''),
